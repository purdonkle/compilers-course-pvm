...
*/
  ...
  // Utilities
    ...
    const int minInt = System.Int32.MinValue;                                               // CHANGES MADE HERE: added smallest int
...
//            if (tracing) results.WriteLine();
          ...
          case PVM.sub:           // integer subtraction
            tos = mem[cpu.sp++]; mem[cpu.sp] -= tos;
            break;
          case PVM.mul:           // integer multiplication                                     CHANGES MADE HERE: multiplicative overflow
            tos = mem[cpu.sp++];    
            long overFlowCheck = (long)tos * (long)mem[cpu.sp]; //multiplicative overflow and underflow check
            ...
            break;
          case PVM.div:           // integer division (quotient)                                 CHANGES MADE HERE: division by zero
            if (mem[cpu.sp] == 0) { // divZero check
                ps = divZero;
            } else {
                tos = mem[cpu.sp++]; mem[cpu.sp] /= tos;
            }
          ...
          case PVM.inc:           // ++                                                             CHANGES MADE HERE: added increment
            cpu.sp++;
            mem[cpu.sp] += 1;
            break;
          case PVM.dec:           // --                                                             CHANGES MADE HERE: added decrement
            cpu.sp++;
            mem[cpu.sp] -= 1;
            break;
