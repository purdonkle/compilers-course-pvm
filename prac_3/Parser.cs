...
public class Parser {
	...
	public const int _page = 1;
	public const int _word = 2;
	// terminals
	public const int EOF_SYM = 0;
	public const int page_Sym = 1;
	public const int word_Sym = 2;
	public const int comma_Sym = 3;
	public const int minus_Sym = 4;
	...
	static void PageNumber() {
		Content();
		while (la.kind == page_Sym) {
			Get();
			while (la.kind == comma_Sym || la.kind == minus_Sym) {
	...
	static void Content() {
		Expect(word_Sym);
		while (la.kind == word_Sym || la.kind == comma_Sym || la.kind == lparen_Sym) {
			if (la.kind == word_Sym) {
				Get();
			} else if (la.kind == comma_Sym) {
				Get();
				Expect(word_Sym);
			} else {
				Get();
